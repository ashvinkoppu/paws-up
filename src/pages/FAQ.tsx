    ...
    <div className="min-h-screen bg-gradient-to-b from-background via-background to-accent/20">
      ...
      <main className="relative z-10 max-w-4xl mx-auto px-4 py-12">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl md:text-5xl font-serif font-bold mb-4">How can we help?</h1>
          <p className="text-muted-foreground text-lg max-w-md mx-auto">Find answers to common questions about caring for your virtual pet.</p>
        </div>
          ...
          {categories.map((cat) => (
            <button
              ...
              className={cn(
                ...
                activeCategory === cat.id ? 'bg-foreground text-background shadow-lg' : 'bg-card/50 text-muted-foreground hover:bg-card hover:text-foreground border border-border/30',
              )}
            >
              <span>{cat.emoji}</span>
              <span>{cat.label}</span>
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isOpen ? 'bg-card border-border/50 shadow-lg' : 'bg-card/30 border-border/20 hover:bg-card/60 hover:border-border/30',
                  )}
                  style={{ animationDelay: `${index * 30}ms` }}
                >
                  <button onClick={() => toggleItem(faq.id)} className="w-full text-left p-5 flex items-start gap-4" aria-expanded={isOpen}>
                    <span className="text-xl mt-0.5 flex-shrink-0">{category?.emoji}</span>
                    <div className="flex-1 min-w-0">
                      <h3 className={cn('font-semibold pr-8 transition-colors', isOpen ? 'text-foreground' : 'text-foreground/80')}>{faq.question}</h3>
                    </div>
                    <ChevronDown className={cn('w-5 h-5 text-muted-foreground flex-shrink-0 transition-transform duration-300', isOpen && 'rotate-180')} />
                  </button>

                  <div className={cn('overflow-hidden transition-all duration-300', isOpen ? 'max-h-[600px] opacity-100' : 'max-h-0 opacity-0')}>
                    <div className="px-5 pb-5 pl-14">
                      <div className="text-muted-foreground leading-relaxed whitespace-pre-line text-[15px]">{renderAnswer(faq.answer)}</div>
                    </div>
                  </div>
                </div>
              );
        ...
        <div className="mt-16 text-center">
          <div className="inline-block p-8 rounded-3xl bg-gradient-to-br from-primary/5 via-accent/30 to-secondary/5 border border-border/20">
            ...
            <p className="text-muted-foreground mb-4">Try our AI assistant or check our policies below.</p>
