...
const ACTIONS = [
  {
    ...
    hoverColor:
      "hover:from-orange-400/40 hover:to-amber-300/35 hover:border-orange-400/50 hover:shadow-orange-400/20 hover:shadow-lg",
  ...
  },
  {
    ...
    hoverColor:
      "hover:from-pink-400/40 hover:to-rose-300/35 hover:border-pink-400/50 hover:shadow-pink-400/20 hover:shadow-lg",
  ...
  },
  {
    ...
    hoverColor:
      "hover:from-yellow-400/40 hover:to-amber-200/35 hover:border-yellow-400/50 hover:shadow-yellow-400/20 hover:shadow-lg",
  ...
  },
  {
    ...
    hoverColor:
      "hover:from-emerald-400/40 hover:to-teal-300/35 hover:border-emerald-400/50 hover:shadow-emerald-400/20 hover:shadow-lg",
  ...
  },
  {
    ...
    hoverColor:
      "hover:from-red-400/40 hover:to-rose-300/35 hover:border-red-400/50 hover:shadow-red-400/20 hover:shadow-lg",
    activeColor: "active:from-red-400/50 active:to-rose-300/45",
  },
];

/** Icon component and past-tense verb shown in the feedback overlay after performing an action. */
const ACTION_FEEDBACK: Record<
  string,
  { icon: React.ComponentType<{ className?: string }>; verb: string }
> = {
  feed: { icon: Utensils, verb: "Fed" },
  play: { icon: Gamepad2, verb: "Played with" },
  rest: { icon: Moon, verb: "Rested" },
  clean: { icon: Sparkles, verb: "Cleaned" },
  vet: { icon: Stethoscope, verb: "Healed" },
  wake: { icon: Sun, verb: "Woke up" },
};
...
const ActionButtons: React.FC = () => {
  ...
  const activeFeedback = feedback
    ...
    : null;
  const activeFeedbackMeta = feedback
    ? ACTION_FEEDBACK[feedback.actionId]
    : null;
    ...
    <div className="glass-card rounded-2xl shadow-md p-5 relative">
      ...
      {feedback && activeFeedback && activeFeedbackMeta && (
        <div
          ...
          style={{
            ...
            animation:
              "actionFeedbackIn 0.2s ease-out, actionFeedbackOut 0.15s ease-in 0.8s forwards",
          }}
        >
          <div
            className={cn(
            ...
            style={{
              minWidth: "320px",
              boxShadow:
                "0 8px 40px rgba(0,0,0,0.12), 0 0 0 1px rgba(255,255,255,0.1) inset",
            }}
          >
            {(() => {
              const FeedbackIcon = activeFeedbackMeta.icon;
              return (
                <div
                  className={cn("p-3 rounded-xl", activeFeedback.bgColor)}
                  style={{ animation: "actionEmojiPop 0.5s ease-out" }}
                >
                  <FeedbackIcon
                    className={cn("w-9 h-9", activeFeedback.color)}
                  />
                </div>
              );
            })()}
            <div className="flex flex-col">
              ...
              <span
                className={cn("text-sm font-semibold", activeFeedback.color)}
              >
      ...
      <div className="grid grid-cols-5 gap-2">
        {ACTIONS.map((action, index) => {
          ...
          let clickAction: "feed" | "play" | "rest" | "clean" | "vet" | "wake" =
            action.id as "feed" | "play" | "rest" | "clean" | "vet" | "wake";
      ...
      <div className="mt-4 pt-3 border-t border-border">
        <p className="text-xs text-muted-foreground text-center flex items-center justify-center gap-1.5">
          ...
          <span>
            {state.petAsleep
              ? "Wake up your pet to play!"
              : "Shop items give bigger stat boosts"}
          </span>
        </p>
      </div>
    </div>
