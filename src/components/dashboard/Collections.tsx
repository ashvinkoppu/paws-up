  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
      {items.map((item) => (
        ...
        >
          <div className="text-4xl mb-3 drop-shadow-sm group-hover:scale-110 transition-transform duration-300">{item.icon}</div>
          <div className="text-center w-full">
            <h4 className="font-semibold text-sm truncate w-full" title={item.name}>
              {item.name}
            </h4>
            <p className="text-[10px] text-muted-foreground line-clamp-2 mt-1 h-8 leading-tight">{item.description}</p>
          </div>
          ...
          {item.passiveEffect && (
            <div className="mt-2 w-full pt-2 border-t border-border/50 flex items-center justify-center gap-1.5 text-[10px] text-emerald-600 font-medium">
              ...
              <span>
                +{item.passiveEffect.bonus} {item.passiveEffect.stat}
              </span>
            </div>
          )}
        </div>
  ...
  return (
    <Card className="h-full border-0 shadow-none bg-transparent">
      <CardHeader className="px-0 pb-4">
        ...
        <CardDescription>Collect rare items, outfits, and themes for your pet!</CardDescription>
      </CardHeader>
      <CardContent className="px-0">
        <Tabs defaultValue="toys" className="w-full">
          <TabsList className="w-full justify-start mb-6 bg-muted/40 p-1">
            <TabsTrigger value="toys" className="flex-1">
              Toys ({toys.length})
            </TabsTrigger>
            ...
            <TabsTrigger value="decor" className="flex-1">
              Decor ({decorations.length})
            </TabsTrigger>
          </TabsList>

          <ScrollArea className="h-[calc(100vh-320px)] pr-4">
            <TabsContent value="toys" className="mt-0">
              <CollectionGrid items={toys} emptyMessage="No toys collected yet. Visit the shop or complete achievements!" />
            </TabsContent>

            <TabsContent value="outfits" className="mt-0">
              <CollectionGrid items={outfits} emptyMessage="No outfits collected yet. Dress up your pet!" />
            </TabsContent>

            <TabsContent value="themes" className="mt-0">
              <CollectionGrid items={themes} emptyMessage="No room themes collected yet. Customize your space!" />
            </TabsContent>

            <TabsContent value="decor" className="mt-0">
              <CollectionGrid items={decorations} emptyMessage="No decorations collected yet." />
            </TabsContent>
          </ScrollArea>
        </Tabs>
      </CardContent>
