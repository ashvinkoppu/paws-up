...
const GameDashboard: React.FC = () => {
  ...
  const [moneyAnimations, setMoneyAnimations] = useState<
    Array<{ id: number; amount: number; swoopX: number; swoopY: number }>
  >([]);
  ...
  const topExpenseEntry = Object.entries(manageSpendingByCategory).sort(
  ...
  useEffect(() => {
    ...
    if (earned > 0) {
      ...
      if (walletRef.current) {
        ...
        swoopY =
          walletRect.top + walletRect.height / 2 - window.innerHeight / 2;
      }
      setMoneyAnimations((previous) => [
        ...previous,
        { id: animationId, amount: Math.round(earned), swoopX, swoopY },
      ]);
      ...
      const cleanupTimer = setTimeout(() => {
        if (isMountedRef.current) {
          setMoneyAnimations((previous) =>
            previous.filter((animation) => animation.id !== animationId),
          );
        }
      }, 1500);
  ...
  const petDisplayPanel = (
    <div className="animate-fade-in-up" data-tutorial="pet-display">
      <PetDisplay
        onXpClick={handleOpenTasks}
        onFinanceClick={handleOpenFinance}
        showFinanceSnapshot={activeScreen === "play"}
      />
    </div>
  );

  const carePanel = (
    <div className="space-y-4" data-tutorial="side-panel">
      <Button
        variant="ghost"
        className="w-full rounded-xl border border-border hover:bg-accent text-foreground flex items-center gap-2"
    ...
    <div className="min-h-screen bg-background relative overflow-x-hidden w-full">
      ...
      <header className="sticky top-0 z-40 bg-background/95 backdrop-blur-sm border-b border-border">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden sm:flex items-center gap-3">
            ...
            <div
              className="flex items-center gap-1.5 px-3 py-1.5 bg-accent border border-border rounded-full text-sm"
              title="Daily Actions Remaining"
            >
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative" ref={notificationPanelRef}>
              <Button
                ...
                className={cn(
                  ...
                  showNotificationPanel ? "bg-primary/10" : "hover:bg-accent",
                )}
              >
                <Bell
                  className={cn(
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent
                align="end"
                className="w-56 rounded-xl p-0 bg-background border border-border shadow-lg overflow-hidden"
              >
                {state.pet && (
                ...
                <div>
                  ...
                  <DropdownMenuItem
                    ...
                    className="rounded-none cursor-pointer py-2.5 px-4 text-amber-600 focus:text-amber-600 focus:bg-amber-500/10"
                  >
                  ...
                  <DropdownMenuItem
                    ...
                    className="rounded-none cursor-pointer py-2.5 px-4 text-muted-foreground focus:text-foreground focus:bg-accent/40"
                  >
        ...
        <AlertDialogContent className="rounded-2xl border-border shadow-lg">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-muted-foreground">
              Are you sure you want to reset your game? All progress will be
              lost, including your pet, money, and achievements.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <main className="max-w-6xl mx-auto px-6 py-8 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-4 space-y-4">{petDisplayPanel}</div>
          ...
          >
            {activeScreen === "play" ? (
                  ...
                  <div className="border border-rose-200 bg-rose-50 dark:bg-rose-950/20 rounded-xl p-3 flex items-center justify-between gap-3 mb-4">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <span className="text-sm text-rose-700 dark:text-rose-400">
                        {state.pet?.name} needs attention: {needsAttentionCount}{" "}
                        {needsAttentionCount === 1 ? "stat is" : "stats are"}{" "}
                        low.
                      </span>
                    </div>
                    <Button
              ...
              <div className="space-y-5">
                  ...
                  <div className="bg-card border border-border rounded-xl p-4">
                    ...
                    <p
                      className={cn(
                        ...
                        state.money < 20 ? "text-rose-600" : "text-foreground",
                      )}
                    >
                      ${state.money.toFixed(0)}
                    </p>
                    ...
                    <div className="mt-2 h-1.5 bg-accent rounded-full overflow-hidden">
                      <div
                        className={cn(
                          ...
                          weeklyBudgetUsedPercent > 100
                            ? "bg-rose-500"
                            : "bg-primary/70",
                        )}
                        style={{
                          width: `${Math.min(weeklyBudgetUsedPercent, 100)}%`,
                        }}
                      />
                    </div>
                    <p className="text-[11px] text-muted-foreground mt-1.5">
                      ${state.weeklySpent.toFixed(0)} of ${state.weeklyBudget}{" "}
                      spent
                    </p>
                  </div>
                ...
                {/* Low-stat alert banner */}
                {needsAttentionCount > 0 && (
                  <div className="border border-rose-200 bg-rose-50 dark:bg-rose-950/20 rounded-xl p-3 flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2 min-w-0">
                      <AlertTriangle className="w-4 h-4 text-rose-500 flex-shrink-0" />
                      <span className="text-sm text-rose-700 dark:text-rose-400">
                        {state.pet?.name} needs attention: {needsAttentionCount}{" "}
        ...
        context={
          state.pet
            ? {
                ...
                careStreak: state.careStreak,
                totalDaysPlayed: state.totalDaysPlayed,
                inventoryCount: state.inventory.reduce(
