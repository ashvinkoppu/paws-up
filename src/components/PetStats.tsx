...
import { Activity, AlertCircle, HelpCircle, TrendingDown, X, History } from 'lucide-react';
...
const STAT_TOOLTIPS: Record<keyof PetStatsType, {
  ...
  health: {
    description: 'Overall health. Critical stat - if it drops too low, your pet gets sick!',
...
const StatBar: React.FC<StatBarProps> = ({ stat, value, previousValue, index, onWhyClick }) => {
      ...
      {showTooltip && (
        <div className="absolute z-50 top-full mt-1 left-0 right-0 p-3 bg-card/98 backdrop-blur-xl rounded-xl border shadow-xl animate-fade-in-up">
          <div className="flex justify-between items-start mb-2">
            ...
            <button onClick={() => setShowTooltip(false)} className="p-0.5 hover:bg-muted rounded-full">
...
const PetStats: React.FC = () => {
  ...
  const [whyModalStat, setWhyModalStat] = useState<keyof PetStatsType | null>(null);
  ...
  const getWhyExplanation = (stat: keyof PetStatsType): string => {
    ...
    // General decay
    reasons.push('Stats naturally decay over time as part of the game mechanics.');
    ...
    switch (stat) {
      case 'hunger':
        ...
        break;
      case 'happiness':
        reasons.push('Happiness drops when your pet is neglected or other stats are low.');
        if (stats.hunger < 30) reasons.push('Low hunger is making your pet unhappy.');
        if (stats.energy < 20) reasons.push('Being tired is affecting their mood.');
        break;
      case 'energy':
        reasons.push('Energy depletes with activities and naturally over time.');
      ...
      case 'cleanliness':
        ...
        break;
      case 'health':
        if (stats.hunger < 30) reasons.push('Low hunger is directly affecting health!');
        if (stats.cleanliness < 30) reasons.push('Poor hygiene is causing health issues!');
      ...
      {showActionLog && (
        <div className="mb-4 p-3 bg-muted/30 rounded-xl border border-border/30 max-h-40 overflow-y-auto">
          ...
          {state.actionLog.length === 0 ? (
            <p className="text-xs text-muted-foreground text-center py-2">No recent actions</p>
          ) : (
            <div className="space-y-1.5">
              {state.actionLog.slice(0, 10).map((log) => (
                <div key={log.id} className="flex items-start gap-2 text-xs">
                  ...
                  <span className="text-muted-foreground/60 text-[10px]">{formatGameTime(log.gameTime)}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fade-in">
          <div className="bg-card rounded-2xl p-5 m-4 max-w-sm w-full shadow-2xl border animate-scale-in">
            <div className="flex justify-between items-start mb-3">
              <div className="flex items-center gap-2">
                <span className="text-2xl">{STAT_CONFIG[whyModalStat].icon}</span>
                <h4 className="font-serif font-bold text-lg">Why is {STAT_CONFIG[whyModalStat].label} low?</h4>
              </div>
              <button onClick={() => setWhyModalStat(null)} className="p-1 hover:bg-muted rounded-full">
                <X className="w-4 h-4 text-muted-foreground" />
              </button>
            </div>
            <p className="text-sm text-muted-foreground mb-4">{getWhyExplanation(whyModalStat)}</p>
            <div className="p-3 bg-secondary/10 rounded-xl border border-secondary/20">
              <p className="text-xs font-medium text-secondary">💡 Tip: {STAT_TOOLTIPS[whyModalStat].increases}</p>
            </div>
            <Button onClick={() => setWhyModalStat(null)} className="w-full mt-4" variant="outline">
              Got it!
            </Button>
          </div>
        </div>
