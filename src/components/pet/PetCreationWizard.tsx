...
import { Species, Personality, PetColor, PetGender, GENDER_COLORS } from '@/types/game';
...
const GENDERS: Array<{
  ...
  { type: 'male', label: 'Male', icon: '♂', description: 'Blues, greens & grays.' },
  { type: 'female', label: 'Female', icon: '♀', description: 'Pinks, purples & peach.' },
  { type: 'neutral', label: 'Neutral', icon: '⚧', description: 'Yellows, teals & cream.' },
];
        ...
        {species ? (
          ...
          <>
            ...
            <div className="text-center">
              ...
              <h3 className="mt-2 font-serif text-3xl font-semibold text-foreground">Choose a pet</h3>
              <p className="mt-1 text-sm text-muted-foreground">Updates as you make selections.</p>
            </div>
          </>
        )}
      </div>
...
const PetCreationWizard: React.FC<PetCreationWizardProps> = ({ onComplete }) => {
  ...
  const selectedSpeciesData = selectedSpecies ? SPECIES_DATA[selectedSpecies] : null;
  const currentPalette = GENDER_COLORS[selectedGender];
  const currentColorHex =
    currentPalette.find((colorOption) => colorOption.color === selectedColor)?.hex ??
          ...
          >
            ...
            <div className="mt-5 rounded-[1.5rem] border border-border bg-background/70 p-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  { label: 'Species', value: selectedSpeciesData?.name ?? 'Not selected' },
                  { label: 'Name', value: petName.trim() || 'Unnamed' },
                  {
                    label: 'Style',
                    value:
                      currentPalette.find((entry) => entry.color === selectedColor)?.name ?? 'Blue',
                  },
                  {
                    label: 'Personality',
                    value: selectedPersonality
                      ? (PERSONALITIES.find((entry) => entry.type === selectedPersonality)?.name ?? 'Selected')
                      : 'Not selected',
                  },
                ].map((item) => (
                  <div
          ...
          >
            ...
            {step === 1 && (
              <div className="grid gap-4 md:grid-cols-2">
                {(
                  Object.entries(SPECIES_DATA) as Array<[Species, (typeof SPECIES_DATA)[Species]]>
                    ...
                    <div className="relative">
                      ...
                      <div className="mt-5 grid items-center gap-4 sm:grid-cols-[96px_minmax(0,1fr)]">
                        <div className="flex h-24 w-24 items-center justify-center rounded-[1.5rem] border border-border bg-card">
                          <img src={data.image} alt={data.name} className="h-16 w-16 object-contain" />
                        </div>
                        <div className="min-w-0">
                          <h3 className="font-serif text-2xl font-semibold text-foreground">{data.name}</h3>
            ...
            {step === 2 && (
              <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_280px]">
                <div className="rounded-[1.6rem] border border-border bg-background/70 p-6">
                  <label htmlFor="pet-name" className="text-sm font-semibold text-foreground">
                    Pet name
                  </label>
                  <p className="mt-2 text-sm leading-6 text-muted-foreground">
                    Keep it short, memorable, and easy to spot in care logs and budget entries.
                  </p>
                  ...
                  <div className="mt-5">
                    ...
                    <div className="mt-3 flex items-center justify-between text-sm">
                      <span className={cn(nameError ? 'text-destructive' : 'text-muted-foreground')}>
                        {nameError || '2 character minimum · Letters and spaces only.'}
                      </span>
                      <span className="text-muted-foreground">{petName.trim().length}/20</span>
                    </div>
                  </div>
                ...
                <div className="rounded-[1.6rem] border border-border bg-card p-6">
                  ...
                  <div className="mt-5 rounded-[1.4rem] border border-border bg-background/70 p-5">
                    <p className="text-sm text-muted-foreground">Your new companion will appear as</p>
                    <h3 className="mt-3 font-serif text-4xl font-semibold tracking-tight text-foreground">
                      {petName.trim() || 'Your pet'}
                    </h3>
                    <p className="mt-3 text-sm leading-6 text-muted-foreground">
                      The name is used across the dashboard, care updates, and spending history.
                    </p>
                  </div>
                  ...
                  {selectedSpeciesData && (
                    <div className="mt-4 rounded-[1.4rem] border border-border bg-background/50 p-4">
                      <p className="text-sm font-semibold text-foreground">
                        Pairing with your {selectedSpeciesData.name.toLowerCase()}
                      </p>
                      <p className="mt-2 text-sm leading-6 text-muted-foreground">
                      ...
                      {GENDERS.map((gender, index) => (
                        ...
                        >
                          <div className="flex items-start gap-3">
                            <span className="text-lg text-foreground/80">{gender.icon}</span>
                            <div>
                              <p className="text-base font-semibold text-foreground">{gender.label}</p>
                    ...
                    <div className="mt-3 grid gap-3 sm:grid-cols-2">
                      {currentPalette.map((colorOption, index) => (
                        ...
                        >
                          ...
                          />
                          <div className="min-w-0">
                            <p className="text-base font-semibold text-foreground">{colorOption.name}</p>
                            <p className="mt-1 text-sm text-muted-foreground">{selectedGender} palette</p>
                          </div>
                          {selectedColor === colorOption.color && (
                  ...
                  <div className="mt-4 grid gap-3">
                    <div className="rounded-2xl border border-border/60 bg-card px-4 py-3">
                      ...
                      <p className="mt-2 text-sm font-semibold text-foreground">
                        {GENDERS.find((entry) => entry.type === selectedGender)?.label}
                      </p>
                    </div>
                    <div className="rounded-2xl border border-border/60 bg-card px-4 py-3">
                      <p className="text-xs font-semibold uppercase tracking-[0.22em] text-muted-foreground">
                      ...
                      <p className="mt-2 text-sm font-semibold text-foreground">
                        {currentPalette.find((entry) => entry.color === selectedColor)?.name}
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  {PERSONALITIES.map((personality, index) => (
                    ...
                    >
                      ...
                      <div className="mt-4 rounded-2xl border border-border/60 bg-card px-4 py-3">
                        ...
                        <p className="mt-2 text-sm leading-6 text-foreground/80">{personality.bestFor}</p>
                      </div>
                    </button>
                  ))}
                </div>
              ...
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                {step >= STEP_CONFIG.length && (
                  <p className="text-sm leading-6 text-muted-foreground">
                    Review the profile on the left, then adopt when everything feels right.
                  </p>
                )}
