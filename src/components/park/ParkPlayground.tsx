...
const STATIONS: Station[] = [
  {
    ...
    groundHue:
      "radial-gradient(ellipse at center, rgba(251,191,36,0.18) 0%, rgba(217,170,60,0.08) 60%, transparent 100%)",
    accentColor: "#F59E0B",
  ...
  },
  {
    ...
    groundHue:
      "radial-gradient(ellipse at center, rgba(249,115,22,0.15) 0%, rgba(234,88,12,0.06) 60%, transparent 100%)",
    accentColor: "#F97316",
  ...
  },
  {
    ...
    groundHue:
      "radial-gradient(ellipse at center, rgba(56,189,248,0.15) 0%, rgba(14,165,233,0.06) 60%, transparent 100%)",
    accentColor: "#0EA5E9",
  ...
  },
  {
    ...
    groundHue:
      "radial-gradient(ellipse at center, rgba(244,114,182,0.15) 0%, rgba(236,72,153,0.06) 60%, transparent 100%)",
    accentColor: "#EC4899",
...
const NatureDetailIcon: React.FC<{ kind: NatureKind; size: number }> = ({
  kind,
  size,
}) => {
  switch (kind) {
...
const CreatureIcon: React.FC<{ kind: CreatureKind }> = ({ kind }) => {
  switch (kind) {
    case "bird":
      return (
        <Bird className="text-amber-700/70" style={{ width: 15, height: 15 }} />
      );
    case "bug":
      return (
        <Bug className="text-red-600/70" style={{ width: 14, height: 14 }} />
      );
    case "butterfly":
      return (
        ...
        <div className="relative" style={{ width: 14, height: 10 }}>
          <div
            style={{
              ...
              borderRadius: "50%",
              background:
                "repeating-linear-gradient(90deg, #FBBF24 0px, #FBBF24 3px, #1F2937 3px, #1F2937 6px)",
              position: "absolute",
              bottom: 0,
              left: 2,
            }}
...
const ParticleIcon: React.FC<{ kind: ParticleKind; color: string }> = ({
  kind,
  color,
}) => {
  ...
  switch (kind) {
    case "heart":
      return <Heart style={iconStyle} fill={color} />;
    case "star":
      return <Star style={iconStyle} fill={color} />;
    case "sparkle":
      return <Sparkles style={iconStyle} />;
    case "paw":
      return <PawPrint style={iconStyle} />;
    case "trophy":
...
const StatPill: React.FC<{
  ...
  return (
    <div
      className="flex items-center gap-2 px-3 py-2 rounded-2xl backdrop-blur-md"
      style={{
        background: accentBg,
    ...
    >
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span
          className="font-serif font-bold text-xs min-w-[20px] text-right"
          style={{ color }}
        >
          {value}
        </span>
...
const ParkPlayground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const updateScale = () => {
      const hudSpace = isMobile ? 220 : 160;
      const padding = 32;
  ...
  const petAnimationClass = (() => {
    switch (petAction) {
      ...
      case "eating":
        return "animate-pet-eating";
      case "jumping":
        return "animate-happy-jump";
      case "playing":
        return "animate-park-run";
      case "socializing":
        return "animate-park-social";
    }
  })();

  // ── Helpers ──

  const triggerPetAction = useCallback(
    (action: PetAction, duration: number) => {
  ...
  const addFloatingText = useCallback(
    (text: string, x: number, y: number, color: string = "text-white") => {
      const id = floatingTextIdRef.current++;
      setFloatingTexts((previous) => [...previous, { id, text, x, y, color }]);
      setTimeout(() => {
        setFloatingTexts((previous) =>
          previous.filter((floatingText) => floatingText.id !== id),
        );
      }, 1500);
    },
    [],
  ...
  const spawnParticles = useCallback(
    (
      x: number,
      y: number,
      items: { kind: ParticleKind; color: string }[],
      count: number = 5,
    ) => {
    ...
    const interval = setInterval(() => {
      setPawPrints((previous) => {
        ...
        const filtered = previous.filter(
          (print) => now - print.timestamp < 3000,
        );
  ...
  const handleFetch = useCallback(() => {
    ...
    spawnParticles(
      ...
      petPosition.y,
      [
        { kind: "wind", color: "#9CA3AF" },
        { kind: "star", color: "#FBBF24" },
      ],
      3,
    );
    addFloatingText(
  ...
  const handleFood = useCallback(() => {
    ...
    const foodItem = state.inventory.find(
      (item) => item.category === "hunger" && item.quantity > 0,
    );
    if (foodItem) {
      ...
      spawnParticles(
        ...
        450,
        [
          { kind: "heart", color: "#EC4899" },
          { kind: "sparkle", color: "#F97316" },
        ],
        6,
      );
      ...
      spawnParticles(
        ...
        450,
        [
          { kind: "dot", color: "#60A5FA" },
          { kind: "sparkle", color: "#38BDF8" },
        ],
        4,
      );
  ...
  const handleNpcInteraction = useCallback(() => {
    const nearNpc = npcs.find((npc) => {
      ...
      return (
        distanceTo(petPosition.x, petPosition.y, npcCurrentX, npcCurrentY) <
        INTERACT_DISTANCE
      );
    });
    ...
    spawnParticles(
      ...
      [
        { kind: "heart", color: "#EC4899" },
        { kind: "paw", color: "#92400E" },
        { kind: "sparkle", color: "#F9A8D4" },
      ],
      5,
    );
    addFloatingText(
      `${interaction.text} +${interaction.happiness}`,
      petPosition.x,
      petPosition.y - 30,
      "text-pink-300",
    );
      ...
      setAgilityState((previous) => {
        ...
        const updatedHurdles = previous.hurdles.map((hurdle) => {
          ...
          if (newPetX > hurdle.x + 25) {
            pendingEffects.push(() =>
              addFloatingText("Miss!", hurdle.x, 350, "text-red-400"),
            );
        ...
        const newScore = updatedHurdles.filter(
        ...
        if (newTimer <= 0 || newPetX > 720) {
          ...
          pendingEffects.push(() => {
            ...
            spawnParticles(
              ...
              450,
              [
                { kind: "trophy", color: "#FBBF24" },
                { kind: "star", color: "#F59E0B" },
                { kind: "sparkle", color: "#FDE68A" },
              ],
              8,
            );
            addFloatingText(
              `+${happinessBonus} Happy!`,
              600,
              420,
              "text-yellow-300",
            );
    ...
    const timer = setInterval(() => {
      setFetchBall((previous) => {
        ...
        if (previous.phase === "landed") {
          const ballX =
            previous.x + (previous.targetX - previous.x) * previous.progress;
          const ballY =
            previous.y + (previous.targetY - previous.y) * previous.progress;
    ...
    const moveInterval = setInterval(() => {
      ...
      if (deltaX !== 0 || deltaY !== 0) {
        ...
        setPetPosition((previous) => ({
          x: Math.max(
            PET_SIZE / 2,
            Math.min(PARK_WIDTH - PET_SIZE / 2, previous.x + deltaX),
          ),
          y: Math.max(
            PET_SIZE / 2,
            Math.min(PARK_HEIGHT - PET_SIZE / 2, previous.y + deltaY),
          ),
        }));
  ...
  useEffect(() => {
    const closest =
      STATIONS.find(
        (station) =>
          distanceTo(petPosition.x, petPosition.y, station.x, station.y) <
          INTERACT_DISTANCE,
      ) || null;
  ...
  const getBallPosition = () => {
    ...
    const arcHeight =
      fetchBall.phase === "flying" ? -80 * Math.sin(progress * Math.PI) : 0;
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background relative overflow-x-hidden select-none">
      {/* Page background decoration */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden -z-0">
        <div className="absolute top-0 right-0 w-[600px] h-[500px] bg-secondary/5 rounded-full blur-3xl -translate-y-1/4 translate-x-1/4" />
        <div className="absolute bottom-0 left-0 w-[500px] h-[400px] bg-primary/5 rounded-full blur-3xl translate-y-1/4 -translate-x-1/4" />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[600px] bg-accent/25 rounded-full blur-3xl" />
      </div>

      {/* ── Accent line ── */}
      <div className="h-0.5 bg-gradient-to-r from-primary/40 via-primary to-primary/40 w-full flex-shrink-0" />

      {/* ── Header ── */}
      <header className="sticky top-0 z-40 bg-background/95 backdrop-blur-sm border-b border-border flex-shrink-0">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          ...
          >
            ...
            <span className="font-serif text-sm font-semibold text-muted-foreground group-hover:text-foreground transition-colors hidden sm:inline">
              Dashboard
            </span>
          </Link>

          {/* Pet name + park title */}
          <div className="flex items-center gap-2 px-3 py-1.5 bg-accent border border-border rounded-full">
            ...
            <Trees className="w-3.5 h-3.5 text-secondary" />
            <span className="font-serif text-xs font-semibold text-muted-foreground hidden sm:inline">
              Paws Park
            </span>
          </div>
          ...
          <div className="flex items-center gap-1.5">
            <StatPill
              icon={Heart}
              value={Math.round(pet.stats.happiness)}
              color="#EC4899"
              accentBg="hsl(var(--accent))"
            />
            <StatPill
              icon={Zap}
              value={Math.round(pet.stats.energy)}
              color="#EAB308"
              accentBg="hsl(var(--accent))"
            />
            <StatPill
              icon={UtensilsCrossed}
              value={Math.round(pet.stats.hunger)}
              color="#F97316"
              accentBg="hsl(var(--accent))"
            />
          </div>
      ...
      {nearStation &&
        ...
        (() => {
          ...
          return (
            ...
            >
              <div
                className="flex items-center gap-4 px-5 py-3.5 rounded-2xl bg-card/95 backdrop-blur-md shadow-lg"
                style={{
                  border: `1px solid ${nearStation.accentColor}30`,
                  boxShadow: `0 12px 40px -8px ${nearStation.accentColor}22, 0 4px 16px -4px rgba(0,0,0,0.08)`,
                }}
              >
                <div
                  className="w-10 h-10 rounded-xl flex items-center justify-center bg-accent border border-border"
                  style={{
                    boxShadow: `0 0 14px 2px ${nearStation.accentColor}12`,
                  }}
                >
                  <StationIcon
                    className="w-5 h-5"
                    style={{ color: nearStation.accentColor }}
                  />
                </div>
                <div>
                  ...
                  <p className="text-xs text-muted-foreground flex items-center gap-1.5 mt-0.5">
                    ...
                    {!isTouchDevice && (
                      <kbd
                        className="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-mono font-bold bg-accent border border-border"
                        style={{ color: nearStation.accentColor }}
                      >
                        SPACE
                      </kbd>
      ...
      {!isTouchDevice && (
        <div className="fixed bottom-6 right-5 z-40 opacity-35 hover:opacity-65 transition-opacity duration-300">
          <div className="grid grid-cols-3 gap-1 text-center">
            <div />
            <div className="w-8 h-8 rounded-lg flex items-center justify-center text-[11px] font-mono font-bold bg-card/90 border border-border text-foreground shadow-sm">
              W
            </div>
            <div />
            <div className="w-8 h-8 rounded-lg flex items-center justify-center text-[11px] font-mono font-bold bg-card/90 border border-border text-foreground shadow-sm">
      ...
      {agilityState?.active && (
        <div className="fixed top-[72px] left-1/2 -translate-x-1/2 z-50">
          <div
            className="flex items-center gap-4 px-5 py-2.5 rounded-2xl bg-sky-500/90 backdrop-blur-sm border border-sky-400/40 shadow-lg"
            style={{
              boxShadow:
                "0 8px 28px -4px rgba(14,165,233,0.35), 0 3px 10px -3px rgba(14,165,233,0.2)",
            }}
          >
            ...
            <div>
              ...
              <p className="text-xs text-white/75 font-mono">
                Score: {agilityState.score}/4 | Time:{" "}
                {agilityState.timer.toFixed(1)}s
              </p>
            </div>
            <div
              className="ml-2 px-3 py-1.5 rounded-xl text-xs font-bold"
              style={{
                background: agilityState.isJumping
                  ? "rgba(74,222,128,0.4)"
                  : "rgba(255,255,255,0.15)",
                color: "white",
                border: agilityState.isJumping
      ...
      <main className="flex-1 flex flex-col items-center justify-center py-3">
        ...
        >
          ...
          >
            ...
            >
              ...
              {/* Dappled Sunlight */}
              <div
                className="absolute inset-0 animate-dappled-shift"
                style={{
                  background: `
            ...
            <div
              ...
              style={{
                boxShadow:
                  "inset 0 0 40px 15px rgba(22,101,52,0.15), inset 0 0 80px 30px rgba(22,101,52,0.06)",
              }}
            />
            ...
            {FLOWERS.map((flower, index) => (
              ...
              >
                <Flower2
                  style={{ width: 16, height: 16, color: flower.variant }}
                />
              </div>
            ))}
                ...
                <div key={station.id}>
                  ...
                  <div
                    ...
                    style={{
                      ...
                      animation: isNear
                        ? undefined
                        : "stationBreath 3.5s ease-in-out infinite",
                    }}
                  >
                    ...
                    />
                    <span
                      className="text-[10px] font-serif font-bold mt-0.5 drop-shadow-sm"
                      style={{
                        color: station.accentColor,
            ...
            {npcs.map((npc) => {
              ...
              const isNear =
                distanceTo(petPosition.x, petPosition.y, currentX, currentY) <
                INTERACT_DISTANCE;
                ...
                >
                  <img
                    ...
                    style={{
                      ...
                      objectFit: "contain",
                      filter:
                        `${npcFilter || ""} drop-shadow(0 2px 4px rgba(0,0,0,0.15))`.trim(),
                    }}
                  />
                  {isNear && !npcCooldowns[npc.id] && (
                    <div
                      className="absolute -top-7 left-1/2 -translate-x-1/2 text-[10px] font-serif font-bold whitespace-nowrap px-2.5 py-1 rounded-full"
                      style={{
                        background:
                  ...
                  {npcCooldowns[npc.id] && isNear && (
                    <div
                      className="absolute -top-5 left-1/2 -translate-x-1/2 text-[9px] text-white/60 whitespace-nowrap"
                      style={{
                        transform: `scaleX(${Math.cos(npc.angle) > 0 ? 1 : -1})`,
                      }}
                    >
              ...
              >
                <div
                  style={{
                    ...
                    borderRadius: "50%",
                    background:
                      "radial-gradient(circle at 35% 35%, #D9F99D, #84CC16, #65A30D)",
                    boxShadow:
                      "0 3px 6px rgba(0,0,0,0.2), inset 0 -2px 4px rgba(0,0,0,0.15)",
                    position: "relative",
                    overflow: "hidden",
                  }}
                >
                ...
                {fetchBall.phase === "landed" && (
                  <div
                    className="absolute -top-6 left-1/2 -translate-x-1/2 text-[10px] font-bold text-green-300 whitespace-nowrap animate-float"
            ...
            >
              {/* Pet shadow */}
              <div
                className="absolute bottom-[-4px] left-1/2 -translate-x-1/2 w-[70%] h-[12%] rounded-full"
              ...
              <div
                ...
                style={{
                  transform: `scaleX(${petDirection === "left" ? -1 : 1})`,
                }}
              >
                <img
                  src={PET_IMAGES[pet.species]}
                  ...
                  style={{
                    filter:
                      `${PET_COLOR_FILTERS[pet.color] || ""} drop-shadow(0 3px 6px rgba(0,0,0,0.2))`.trim(),
                  }}
                />
              </div>
              {/* Pet name label */}
              <div
                className="absolute -top-6 left-1/2 -translate-x-1/2 text-[10px] font-serif font-bold whitespace-nowrap px-2.5 py-0.5 rounded-full shadow-md"
                style={{
                  background:
                    "linear-gradient(135deg, hsl(12 76% 50% / 0.85), hsl(12 76% 55% / 0.85))",
                  color: "white",
                  boxShadow:
                    "0 2px 8px -2px rgba(0,0,0,0.2), inset 0 1px 0 rgba(255,255,255,0.15)",
                }}
              >
                {pet.name}
              </div>
            ...
            {floatingTexts.map((floatingText) => (
              <div
                key={floatingText.id}
                className={cn(
                  "absolute pointer-events-none z-40 font-serif font-bold text-sm",
                  floatingText.color,
                )}
                style={{
                  left: floatingText.x,
            ...
            {agilityState?.active && (
              <div className="absolute inset-0 z-20 pointer-events-none">
                ...
                <div
                  ...
                  style={{
                    background:
                      "linear-gradient(180deg, rgba(56,189,248,0.08), rgba(56,189,248,0.18), rgba(56,189,248,0.08))",
                ...
                {agilityState.hurdles.map((hurdle, index) => (
                  ...
                  >
                    <div
                      style={{ position: "relative", width: 34, height: 40 }}
                    >
                      <div
                        style={{
                          position: "absolute",
                          width: 4,
                          height: 40,
                          background: hurdle.missed ? "#EF4444" : "#92400E",
                          borderRadius: 2,
                          left: 2,
                          bottom: 0,
                        }}
                      ...
                      <div
                        style={{
                          ...
                          borderRadius: 3,
                          top: 4,
                          background: hurdle.cleared
                            ? "#22C55E"
                            : hurdle.missed
                              ? "#EF4444"
                              : "#F97316",
                          boxShadow: hurdle.cleared
                      ...
                      {hurdle.missed && (
                        <div className="absolute -top-5 left-1/2 -translate-x-1/2 text-xs text-red-500">
                          &#10007;
                        </div>
                      )}
                    </div>
                ...
                <div
                  ...
                  style={{
                    ...
                    transform: agilityState.isJumping
                      ? "translateY(-28px)"
                      : "translateY(0)",
            ...
            <div
              ...
              style={{
                background:
                  "radial-gradient(ellipse 80% 80% at 50% 50%, transparent 50%, rgba(22,101,52,0.08) 100%)",
              }}
            />
          </div>
        </div>
        ...
        <div className="z-10 mt-3">
          <div className="flex items-center gap-2.5 px-4 py-2 bg-card/80 backdrop-blur-md border border-border rounded-2xl shadow-sm">
            <Trees className="w-3.5 h-3.5 text-secondary" />
            <div className="w-px h-3.5 rounded-full bg-border" />
        ...
        <>
          ...
          >
            <div className="relative" style={{ width: 140, height: 140 }}>
              {/* Up */}
              <button
                className="absolute flex items-center justify-center rounded-2xl active:scale-90 transition-all duration-100"
                style={{
                  top: 0,
                  left: "50%",
                  transform: "translateX(-50%)",
                  width: 48,
                  height: 48,
                  background: activeTouches.has("up")
                    ? "hsl(var(--primary) / 0.15)"
                    : "hsl(var(--card) / 0.9)",
                  border: activeTouches.has("up")
                    ? "1.5px solid hsl(var(--primary) / 0.4)"
                    : "1.5px solid hsl(var(--border))",
                  backdropFilter: "blur(10px)",
                  boxShadow: "0 2px 8px hsl(var(--shadow) / 0.12)",
                }}
                onTouchStart={(event) => {
                  event.preventDefault();
                  handleTouchDirection("up", true);
                }}
              ...
              <button
                ...
                style={{
                  ...
                  width: 48,
                  height: 48,
                  background: activeTouches.has("down")
                    ? "hsl(var(--primary) / 0.15)"
                    : "hsl(var(--card) / 0.9)",
                  border: activeTouches.has("down")
                    ? "1.5px solid hsl(var(--primary) / 0.4)"
                    : "1.5px solid hsl(var(--border))",
                  backdropFilter: "blur(10px)",
                  boxShadow: "0 2px 8px hsl(var(--shadow) / 0.12)",
                ...
                onTouchStart={(event) => {
                  event.preventDefault();
                  handleTouchDirection("down", true);
                }}
              ...
              <button
                ...
                style={{
                  ...
                  width: 48,
                  height: 48,
                  background: activeTouches.has("left")
                    ? "hsl(var(--primary) / 0.15)"
                    : "hsl(var(--card) / 0.9)",
                  border: activeTouches.has("left")
                    ? "1.5px solid hsl(var(--primary) / 0.4)"
                    : "1.5px solid hsl(var(--border))",
                  backdropFilter: "blur(10px)",
                  boxShadow: "0 2px 8px hsl(var(--shadow) / 0.12)",
                ...
                onTouchStart={(event) => {
                  event.preventDefault();
                  handleTouchDirection("left", true);
                }}
              ...
              <button
                ...
                style={{
                  ...
                  width: 48,
                  height: 48,
                  background: activeTouches.has("right")
                    ? "hsl(var(--primary) / 0.15)"
                    : "hsl(var(--card) / 0.9)",
                  border: activeTouches.has("right")
                    ? "1.5px solid hsl(var(--primary) / 0.4)"
                    : "1.5px solid hsl(var(--border))",
                  backdropFilter: "blur(10px)",
                  boxShadow: "0 2px 8px hsl(var(--shadow) / 0.12)",
                ...
                onTouchStart={(event) => {
                  event.preventDefault();
                  handleTouchDirection("right", true);
                }}
              ...
              {/* Center indicator */}
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-border/30" />
            </div>
          </div>
          ...
          <button
            ...
            style={{
              ...
              boxShadow: nearStation
                ? `0 8px 28px -4px ${nearStation.accentColor}45, 0 4px 12px -4px ${nearStation.accentColor}25`
                : agilityState?.active
                  ? "0 8px 28px -4px rgba(14,165,233,0.4), 0 4px 12px -4px rgba(14,165,233,0.2)"
          ...
          >
            {agilityState?.active ? (
              ...
              <div className="flex flex-col items-center">
                ...
                <span className="text-white font-bold text-[9px] mt-0.5">
                  ACTION
                </span>
              </div>
            ) : (
